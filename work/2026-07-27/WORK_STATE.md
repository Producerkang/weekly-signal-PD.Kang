...
- `06_deep_dive/ARTICLE.md`: PENDING
- 심화 유형: 구조 해부형
...
`국가별 이해관계 형성 → 메르코수르 공동협상·합의 → 공동서명 → 각국 국내 승인·비준 → 국가별 발효 가능성 → 최종 협정문에서 발효·양허 주체 확인`

- 일반 Politics 기사의 3갈래 구조는 반복하지 않는다.
- 싱가포르 사례는 공동협정이 국가별 비준 속도에 따라 서로 다른 날짜에 발효된 실제 증거로만 사용한다.
- 한·메르코수르가 같은 발효 조항을 택할 것처럼 예측하지 않는다.
- 볼리비아는 회원국 역사 설명으로 확장하지 않고 최종 서명·발효 주체의 범위 예외로만 배치한다.
- 결말은 협상 타결·서명·국내 승인·발효·관세철폐 시작을 서로 다른 시간표로 구분하는 구조로 회수한다.

## 다음 작업

`06_deep_dive/ARTICLE.md`를 작성한다.

### DEEP_DIVE ARTICLE 기준

1. FLOW 순서에 따라 처음부터 새로 쓰고 VERIFY 문장을 복사하지 않는다.
2. 상단 라벨은 `DEEP DIVE · POLITICS / 한·메르코수르 발효 구조` 계열로 잡되 최종 문구는 본문 완성 뒤 확정한다.
3. 첫머리는 일반 Politics 기사를 다시 요약하지 않고 메르코수르 공동협상이 실제 효력으로 바뀌는 단계에서 시작한다.
4. 설명 단위 1~3에서 국내 입장·공동협상·공동서명과 비준 구조를 선형적으로 설명한다.
5. 싱가포르 사례는 설명 단위 4에서만 실제 사례로 사용하고, 한·메르코수르의 미래를 예측하는 근거로 확대하지 않는다.
6. 볼리비아는 협상·서명·발효 주체의 범위 예외로 짧고 정확하게 처리한다.
7. NARRATIVE와 ANALYSIS를 닫은 뒤 COHERENCE PASS와 STYLE & ARGUMENT PASS를 수행한다.
8. 제목과 Deck은 마지막에 확정한다.
9. 기사에 직접 사용한 주요 출처만 ARTICLE 말미에 남긴다.
10. ARTICLE REVIEW 통과 후에만 DEEP_DIVE를 COMPLETE로 변경한다.

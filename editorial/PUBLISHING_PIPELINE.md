...
- 실패본·연락시트·스토리보드·중간 이미지 후보를 `archive/`에 남기지 않는다.
...
이미지와 HTML은 완전히 직렬로 묶지 않는다.

- 이미지 슬롯과 지면 구조가 확정되면 HTML/CSS 구조 작업을 진행할 수 있다.
- 통과한 이미지는 순차적으로 HTML에 반영할 수 있다.
- 한 이미지 실패 때문에 다른 이미지 제작과 HTML 전체가 멈추지 않도록 한다.
- 다만 모든 필수 이미지가 반영되고 실제 화면 검수를 통과하기 전에는 발행하지 않는다.

## 3. 이미지 제작

이미지 생성은 `editorial/IMAGE_PIPELINE.md`를 따른다.

핵심:

- 한 번에 이미지 한 장만 생성
- 각 슬롯 독립 상태 관리
- 제1호 및 현행 시연 수준의 사진적·에디토리얼 품질이면 통과 가능
- 기사와 자연스럽게 연결되면 충분하며 문자 그대로 전체 메커니즘을 재현하지 않음
- Cover 장변 2200px 이상 목표
- 나머지 주요 이미지 장변 2000px 이상 목표
- LIFE SCENE만 4:3 또는 4:5 비율을 생성 전에 확정
- Politics와 Politics DEEP DIVE는 완전 무인
- 한 슬롯 기본 최대 3회 시도
- 실패 슬롯은 BLOCKED로 남기고 다른 작업 계속 가능
...
이미지 실패를 임시 그래픽으로 우회하지 않는다. 반대로 한 이미지 실패 때문에 이미 통과한 다른 이미지를 다시 만들거나 전체 HTML 제작을 정지시키지도 않는다.

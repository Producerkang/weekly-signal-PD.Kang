...
이 문서는 한 호를 조사·작성·편집·검수·발행하는 실행 순서를 규정한다. GitHub Actions는 사용하지 않는다. 최종 통과 여부는 자동 검사나 사용자 사전 승인보다 제작자의 직접 편집 검수로 결정한다.
...
- 조사 메모·원고 초안·이미지 후보·중간 HTML은 GitHub 저장소 밖의 작업 환경에서 처리한다.
- 완성 전 상태를 `main`에 올리지 않는다.
- 원고·이미지·지면·반응형 화면 검수가 모두 끝난 완성본만 `main`에 직접 반영한다.
- 새 회차 시작 시 직전 회차와 실패 기록을 검토한다.
- 이번 회차에서 개선할 원고·이미지·지면 목표를 각각 하나 이상 정한다.
- 직전 회차보다 나아진 점을 설명할 수 없는 결과는 완성본으로 보지 않는다.
...
8. 직전 회차보다 정보 밀도·논점 선명도·출처 다양성 중 하나 이상을 개선한다.

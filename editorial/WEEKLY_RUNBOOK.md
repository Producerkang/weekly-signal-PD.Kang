...
LIFE SCENE 이미지의 일반 기본값은 세로 `4:5`다. 해당 회차 지면이 넓은 스프레드나 가로 장면을 명확히 요구할 때만 회차별 `LAYOUT_PLAN.md`와 `IMAGE_PLAN.md`에서 `4:3`으로 오버라이드한다.
...
- 특정 과거 회차를 영구 품질 기준으로 삼지 않음
- 기사와 자연스럽게 연결되고 실제 지면에서 기능하면 합격 가능
- 기사 전체 메커니즘을 한 장에 재현하지 않음
- Cover 장변 2200px 이상 목표
- 나머지 주요 이미지 장변 2000px 이상 목표
- LIFE SCENE 기본 비율은 4:5, 회차 지면상 필요가 명확할 때만 4:3 오버라이드
...
과거 회차는 참고 사례일 뿐 영구 계약 기준이 아니다. 동일 이미지 재사용이나 낮은 품질의 자산을 관성적으로 계승하지 않는다.

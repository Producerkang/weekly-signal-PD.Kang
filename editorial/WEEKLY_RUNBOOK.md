...
이미지 제작 턴은 **현재 기사 슬롯에 직접 넣을 독립된 단일 에디토리얼 이미지 1장**을 만드는 작업이다.

`WORK_STATE.md`, 저장소 구조, 파일 트리, 체크리스트, 진행률, 다음 턴 계획은 이미지 생성의 입력·제어 정보일 뿐 시각 소재가 아니다.
...
- 이미 합격권인 이미지를 취향상 더 좋은 결과를 찾기 위해 반복 생성하지 않음

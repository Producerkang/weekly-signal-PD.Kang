...
- `work/YYYY-MM-DD/` = 제작 상태와 검증·원고 작업
...
`VERIFY.md`와 `ARTICLE.md`의 역할은 `ARTICLE_WRITING_STANDARD.md`를 따른다. 내부 검증 언어와 독자용 최종 원고를 같은 파일에 섞지 않는다.
...
4. 검증이 충분하면 `ARTICLE.md` 작성
5. 독자용 설명을 먼저 완결
6. 기사 후반에 필요한 분석·한계·부작용을 보강
7. 부정형·대조형 문체가 기본 전개가 되었는지 STYLE PASS
8. 마지막에 제목과 Deck 작성
9. ARTICLE REVIEW
10. 통과하면 `COMPLETE`
...
모든 일반 기사가 동일한 소제목 수와 문단 리듬을 갖지 않게 한다. 본문이 디자인·카드·이미지 없이도 충분히 읽힐 때만 COMPLETE로 인정한다.
...
수정이 필요하면 해당 기사만 `IN_REVIEW`로 되돌린다.

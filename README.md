...
- `지면 설계`는 **월요일 07:00의 독립 턴**입니다.
- `이미지 슬롯별 순차 제작`은 **월요일 08:00의 독립 턴**입니다.
- `HTML 제작 + 간단 화면 검수 + 발행`은 마지막 한 턴에서 처리합니다.

지면 설계와 이미지 제작은 서로 다른 대화·예약 작업으로 분리합니다. 이미지 생성 과정에서 오류가 발생해도 07:00에 확정한 지면 설계는 독립적으로 보존되어야 합니다.

### 07:00 지면 설계

07:00 작업은 이미지 생성이 아니라 **지면과 이미지 실행 계획을 확정하는 텍스트·파일 작업**입니다.

이 턴의 필수 산출물:

- `LAYOUT_PLAN.md` — 최종 DOM, 기사별 지면 리듬, 이미지 슬롯과 배치 역할
- `IMAGE_PLAN.md` — 슬롯별 필요성·비율·해상도·장면 브리프·상태와 **scene-only 생성 입력**
- `WORK_STATE.md` — `LAYOUT: COMPLETE`, 다음 작업을 08:00 이미지 제작으로 갱신

07:00 턴에서는 **이미지 생성 도구를 호출하지 않습니다.** 이미지가 필요한 슬롯마다 실제 세계의 한 장면을 먼저 설계하고, 08:00 턴이 원고 전체나 지면 설계를 다시 해석하지 않아도 되도록 `IMAGE_PLAN.md`를 `READY` 상태로 완성합니다.

각 이미지 슬롯에는 이미지 생성용 **`SCENE_ONLY_PROMPT`**를 둡니다. 이 블록에는 장면·피사체·카메라·빛·재질·구도·비율과 필요한 하드 규칙만 남기고 다음 운영 메타데이터는 넣지 않습니다.

- WEEKLY SIGNAL 프로젝트명·브랜드명
- 회차 번호·날짜·예약 시각
- GitHub·저장소·브랜치·커밋·파일 경로
- WORK_STATE·진행률·체크리스트
- 기사 제목·섹션 라벨을 그대로 그리라는 지시
- 다음 턴·완료 보고·제작 상태 문구

### 08:00 이미지 슬롯별 순차 제작

08:00 작업은 **이미 완성된 지면 설계를 입력으로 받는 이미지 제작 전용 턴**입니다. 이 턴에서 지면을 다시 설계하거나 `LAYOUT_PLAN.md`를 다시 작성하지 않습니다.
...
이미지 생성 실행 단위는 항상 슬롯 하나입니다.

```text
1 SLOT = 1 SCENE = 1 IMAGE
```

한 예약 작업 턴 안에서 각 슬롯을 순서대로 처리합니다. 각 슬롯마다 **현재 슬롯 블록 확인 → SCENE_ONLY_PROMPT 확인 → 이미지 한 장 생성 → 판정 → 상태 기록 → 다음 슬롯**을 반복합니다. 한 번의 생성 요청에서 여러 슬롯을 동시에 만들지 않습니다.

08:00 턴은 기본적으로 `IMAGE_PLAN.md`가 이미 준비한 장면을 사용합니다. 이미지 생성 직전 원고 전체, 저장소 파일 트리, 지면 설계 과정, 작업 보고서를 다시 시각 문맥으로 쌓지 않습니다. 슬롯 계획이 불완전하면 임의로 업무 화면을 시각화하지 말고 해당 슬롯을 계획 불완전 상태로 기록합니다.

08:00 턴에서 생성되는 이미지는 **실제 기사 지면에 사용할 독립 에디토리얼 이미지 후보뿐**입니다. 저장소 확인 화면, WORK_STATE, 지면 설계, IMAGE_PLAN, 체크리스트, 진행률, 작업 결과 보고, 다음 턴 안내를 이미지로 만들지 않습니다.

이미지별 세부 상태·품질·실패·재시도는 `editorial/IMAGE_PIPELINE.md`를 따릅니다.
...
- `IMAGE_PLAN.md` — 해당 회차 이미지 슬롯·scene-only 생성 입력·상태
...
그 다음 `WORK_STATE.md`가 가리키는 **현재 예약 작업에 필요한 계약만** 읽습니다. 이미 완료된 이전 턴의 설계 과정을 새 대화에 불필요하게 다시 쌓지 않습니다.

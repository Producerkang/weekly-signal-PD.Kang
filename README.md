...
이미지는 회차 `LAYOUT_PLAN.md`와 `IMAGE_PLAN.md`에서 필요한 슬롯을 확정한 뒤, **하나의 이미지 제작 턴 안에서 각 슬롯 이미지를 각각 생성·판정**합니다. 이미지별 상태와 실패 처리는 `editorial/IMAGE_PIPELINE.md`를 따릅니다.

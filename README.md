...
- `지면 설계 + 이미지 입력 준비`는 월요일 07:00의 독립 턴입니다.
- `이미지 슬롯별 순차 제작`은 월요일 08:00의 독립 턴입니다.
- `HTML 제작 + 간단 화면 검수 + 발행`은 마지막 한 턴에서 처리합니다.

### 07:00 지면 설계 + 이미지 입력 준비

07:00은 **텍스트·파일 작업 전용**입니다.
...
`IMAGE_PIPELINE.md`와 `IMAGE_DIRECTION.md`은 레거시 호환용 안내 파일이며 현행 이미지 계약이 아닙니다.

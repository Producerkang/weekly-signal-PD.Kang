...
- `지면 설계`는 독립 턴으로 완료하고 `LAYOUT_PLAN.md`와 이미지 슬롯 구조를 확정합니다.
- `전체 이미지 제작`은 지면 설계가 완료된 다음 독립 턴에서 수행합니다.
- `HTML 제작 + 간단 화면 검수 + 발행`은 마지막 한 턴에서 처리합니다.

지면 설계와 이미지 제작은 실행 안정성을 위해 분리합니다. 이미지 생성 과정에서 오류나 도구 실패가 발생하더라도 이미 완료된 지면 설계와 `LAYOUT_PLAN.md`는 독립적으로 보존되어야 합니다.

### 08:00 전체 이미지 제작

월요일 08:00 작업은 완료된 `LAYOUT_PLAN.md`와 `IMAGE_PLAN.md`를 기준으로 해당 회차에 필요한 이미지 슬롯 전체를 처리합니다.
...
하나의 예약 작업 턴 안에서 각 슬롯을 순서대로 처리합니다. 각 슬롯마다 **한 장 생성 → 판정 → 상태 기록 → 다음 슬롯**의 순서를 반복하며, 한 번의 이미지 생성 요청에서 여러 슬롯을 동시에 만들지 않습니다. 한 슬롯의 실패가 다른 슬롯의 순차 진행을 막지 않으며, `RETRY` 또는 `BLOCKED` 판정 뒤 다음 처리 가능한 슬롯으로 넘어갑니다. 이미지별 세부 상태와 실패 처리는 `editorial/IMAGE_PIPELINE.md`를 따릅니다.

### 08:00 이미지 생성 도구 호출 게이트

08:00 작업이 시작됐다고 해서 바로 이미지 생성 도구를 호출하지 않습니다.

다음 단계는 **텍스트·파일 작업 전용**이며 이미지 생성 도구 호출을 금지합니다.

- 저장소와 `WORK_STATE.md` 확인
- 현행 계약 확인
- 완료된 `LAYOUT_PLAN.md` 확인
- `IMAGE_PLAN.md` 확인·수정
- 처리할 슬롯 선택
- 기사·섹션 읽기
- `GENERATION BRIEF` 작성
...
이미지 생성 도구는 **현재 슬롯의 `GENERATION BRIEF`가 완성되고 `GENERATION BRIEF READY`로 판정된 직후에만** 호출할 수 있습니다. 이때 한 번의 호출은 해당 슬롯의 독립 에디토리얼 이미지 한 장만 생성합니다.

이미지가 한 장 반환되면 즉시 이미지 생성 단계에서 빠져나와 텍스트·파일 문맥에서 판정과 상태 기록을 수행합니다. 다음 슬롯 이미지는 그 슬롯의 새 `GENERATION BRIEF`가 다시 준비된 뒤에만 생성합니다.

따라서 08:00 턴에서 생성되는 이미지는 **실제 기사 지면에 사용할 슬롯별 이미지 후보뿐**입니다. 저장소 확인 화면, WORK_STATE, 지면 설계, IMAGE_PLAN, 체크리스트, 진행률, 작업 결과 보고, 다음 턴 안내를 이미지로 시각화하지 않습니다.
